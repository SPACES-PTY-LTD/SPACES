...
import type { Location, VehicleActivity } from "@/lib/types"
...
export function LocationDialogContent({
  ...
  const [activities, setActivities] = React.useState<VehicleActivity[]>([])
  const [locationActivityCount, setLocationActivityCount] = React.useState(0)
  const [outboundShipmentCount, setOutboundShipmentCount] = React.useState(0)
  const [inboundShipmentCount, setInboundShipmentCount] = React.useState(0)
  ...
  const loadLocation = React.useCallback(async () => {
    ...
    const [locationResponse, activitiesResponse, shipmentsFromResponse, shipmentsToResponse] =
      await Promise.all([
    ...
    if (isApiErrorResponse(locationResponse)) {
      setLocation(null)
      setActivities([])
      setLocationActivityCount(0)
      setOutboundShipmentCount(0)
      setInboundShipmentCount(0)
      setError(locationResponse.message)
      setLoading(false)
      return
    }

    const nextActivities = !isApiErrorResponse(activitiesResponse)
      ? activitiesResponse.data ?? []
      : []
    const nextLocationActivityCount = !isApiErrorResponse(activitiesResponse)
      ? Number(activitiesResponse.meta?.total ?? nextActivities.length)
      : 0
    const nextOutboundShipmentCount = !isApiErrorResponse(shipmentsFromResponse)
      ? Number(
          shipmentsFromResponse.meta?.total ?? shipmentsFromResponse.data?.length ?? 0
        )
      : 0
    const nextInboundShipmentCount = !isApiErrorResponse(shipmentsToResponse)
      ? Number(shipmentsToResponse.meta?.total ?? shipmentsToResponse.data?.length ?? 0)
      : 0

    setLocation(locationResponse)
    setActivities(nextActivities)
    setLocationActivityCount(nextLocationActivityCount)
    setOutboundShipmentCount(nextOutboundShipmentCount)
    setInboundShipmentCount(nextInboundShipmentCount)
    setLoading(false)
  }, [accessToken, locationId, merchantId])
