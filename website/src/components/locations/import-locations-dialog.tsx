...
import { importLocationsCsv, type ImportLocationsResult } from "@/lib/api/locations"
...
export function ImportLocationsDialog({
  ...
  const [result, setResult] = React.useState<ImportLocationsResult | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const response = await importLocationsCsv(
      ...
      toast.success("Location import completed.")
      router.refresh()
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Import locations via CSV</DialogTitle>
          <DialogDescription>
            Upload a CSV or TXT file to bulk create and update locations.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border bg-muted/20 p-3 text-sm">
            ...
            <Link
              href="/samples/locations-import-sample.csv"
