...
Admin geofence names (1.32, implemented locally): hovering over a displayed polygon or radius boundary shows a compact name tooltip beside the pointer. Use location name, then company/code, then “Unnamed geofence”. Render plain text; the tooltip must not intercept pointer events. Hide it on pointer exit, map dragging/zooming, toggle-off and cleanup. No mobile/Figma changes; live visual verification pending.
